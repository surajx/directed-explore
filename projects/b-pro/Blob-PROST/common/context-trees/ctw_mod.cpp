...
CTNode::CTNode() : m_log_prob_est(0.0), m_log_prob_weighted(0.0) {
  m_count[0] = 0;
  m_count[1] = 0;
  m_child[0] = NULL;
  m_child[1] = NULL;
...
void CTNode::updateWeighted() {
  ...
  // NOTE: no need to compute the log(1+e^x) if x is large, plus it avoids
  // overflows
  if (log_one_plus_exp < 100.0)
    log_one_plus_exp = std::log(1.0 + std::exp(log_one_plus_exp));
...
weight_t CTNode::logProbEstimated() const {
  if (UseZeroRedundancy) {
    if (m_count[0] + m_count[1] == 0)
      return 0.0;
    double rval = log_point_five + m_log_prob_est;
    if (m_count[0] == 0)
      rval = logAdd(log_quarter, rval);
    if (m_count[1] == 0)
      rval = logAdd(log_quarter, rval);
    return rval;
  }
...
void ContextTree::deleteCT(CTNode* n) {
  ...
  if (n->m_child[0] != NULL)
    deleteCT(n->m_child[0]);
  if (n->m_child[1] != NULL)
    deleteCT(n->m_child[1]);
...
double ContextTree::prob(bit_t b) {
  ...
  for (size_t i = 0; i < created.size(); ++i)
    m_ctnode_pool.free(created[i]);
  for (size_t i = 0; i < modified.size(); ++i)
    *modified[i].first = modified[i].second;

  return rval;
}

/* the probability of seeing a particular symbol next */
double ContextTree::logProb(bit_t b) {
  typedef std::pair<CTNode*, CTNode> ctpair_t;

  double before = logBlockProbability();

  // compute the current contexts
  context_t context;
  getContext(m_history, context);
  ...
  double rval = logBlockProbability() - before;
